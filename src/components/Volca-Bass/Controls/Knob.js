...
function Knob(props){
    const {type, knobIsClicked, size} = props;
    const knobClass = `${type}`;
    const overlay = `${type} over knob`;
    const ref = useRef();
    
    
    function handleKnobSize(){

        if(size === 'big'){
            return(
                <svg className={ knobClass } ref={ ref } width='75' height='75' viewBox='0 0 75 75' fill='none' xmlns='http://www.w3.org/2000/svg'>
           ...
           return(
                <svg className={ knobClass } ref={ ref } width='38' height='37' viewBox='0 0 38 37' fill='none' xmlns='http://www.w3.org/2000/svg'>
           ...
           return( 
            <svg className={ knobClass }  ref={ ref } width='20' height='20' viewBox='0 0 20 20' fill='none' xmlns='http://www.w3.org/2000/svg'>
    ...
    return(
            ...
            <div 
                className={ overlay } 
                onMouseDown={()=> knobIsClicked(ref.current,'down')}
                onMouseUp={()=> knobIsClicked(ref.current,'up')}
                >
